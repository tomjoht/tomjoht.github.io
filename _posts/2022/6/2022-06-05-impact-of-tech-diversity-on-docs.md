...
In the [previous post](/trends/embracing-diversity-pluralism-fragmentation.html), I argued that diversity/pluralism/fragmentation define the most prominent trajectories of technology, but I didn't delve into the specific impact on documentation. That's what I'll tackle in this post.

What's the impact of pluralism/diversity/fragmentation on documentation? When you have a large developer portal with content contributed by many different siloed, niche engineering teams working on different aspects of the solution, here's what happens: the developer portal loses the big picture. This is because each niche team sees and understands only their sliver of the solution.

For example, a team producing a Gizmo API has specialists who understand and convey the details, nuances, and sample code for the Gizmo API. But how does the Gizmo API fit into the larger product solution? How does it interact with other APIs? In what ways is the Gizmo API part of a larger number of APIs that developers must use to achieve their end goals? The Gizmo API team probably won't be able to tell you this. They can only share the Javadocs for the Gizmo API with you to publish.
...
In software organizations producing large products (for example, Fire TV at Amazon, Google Maps at Google, and so on), the products have so many hundreds of people working on them, each focusing on a different aspect, it's the equivalent of Smith's 18 different specialized groups operating on different aspects of the pin, none of which understand how all of these tasks come together as a whole. It's a miracle that products have coherence at all, given that few people understand how everything fits together as a whole, or how the parts interact when owned by different organization units, or other big picture understanding.

The big-picture knowledge probably resides in the heads of a handful of engineering directors and high-level product managers, but it's missing across the swaths of independent engineering teams so steeped in specialized knowledge of their stewardship that they know and understand their role only, with a vague understanding of how it fits into the larger picture. These massive products almost take on a life and momentum of their own.

In no possible way could you have one software engineer responsible for the entire product, at least not at this point in the maturity and robustness of the product. Instead, there are specialists all along the way handling every aspect of the solution. And one of those specialists is the technical writer, responsible for documentation on its implementation. Here's where I think there's an opportunity that's missed. In this role of ever-increasing specialization of roles and stewardships with the product, who understands the big picture? Who can look at an article that an engineering team wants to contribute to a developer portal and see how the information fits or conflicts with 10,000 other pages on the developer portal? Who can understand how the Gizmo API gets implemented in five different customer scenarios, and what other dependencies and APIs developers are also using? The opportunity to “specialize” in big picture understanding is a role ripe for technical writers to fill.

When technical writers understand the big picture, they can shape the information architecture of developer portals with a coherent arc that makes the information more than just a collection of independent wikipedia articles. The arc can model the information along the developer's journey, from start to finish. Structuring information within the trajectory of the developer's journey requires tech writers to understand that journey.

As evidence for how compelling the big picture can be, you need only walk through IKEA, the Swedish furniture store. What makes IKEA so unique? It's not the wordless instruction manuals (which are no doubt a wonder), or the untranslated Swedish product names for everything (e.g., “Hemnes” for TV stand, “Khallax” for bookshelf), or the fact that prices are cheap because you have to assemble everything for hours on end. What's truly magical about IKEA is the furnished demo rooms that show the full context of products. Yesterday my daughter begged me to take her to IKEA to get a new dresser that would fit into her closet, and we spent the first hour at IKEA marveling at all the completely furnished demo rooms depicting how each of their products might fit into the larger context of a house.
...
<img src="https://s3.us-west-1.wasabisys.com/idbwmedia.com/images/ikeakitchenstudio1.jpg" alt="IKEA studio" />
...
These rooms are fascinating. It's so much more interesting to see the products working together like this rather than sitting in groups in individual shelves. My daughter says that IKEA inspired her to want to become an architect, and she's already spend half of her youth designing house after house in Roblox and Minecraft.

In some cases, to demonstrate a room, IKEA designers have build pseudo staircases, or added landscapes outside fake windows, or have added many products they don't actually sell (guitars, shoes, tools) to simulate the scene.
...
What's simply interesting is to see the vision of how everything normally sold as a separate item fits together into one larger story.
...
In the same way, documentation should structure content along user journeys, and show furnished rooms rather than merely shelves of APIs.

Creating full-picture documentation isn't necessarily easy. I've written about this in the past with some previous posts:
...
The problem with this approach is that it might come across as irrelevant for anyone not wanting to draw a black cat. For example, maybe I just want to create the white reflection in the eye pupils. Must I watch an entire video tutorial spanning 10+ techniques to get to the one part I need? This bursting of long, sequential material into individual, modular topics was heralded as one of the advancements of tech comm. This modularity allowed users to jump to the exact task or topic they wanted to learn about.

In the same way, if you go into IKEA and you want to buy some cubby bins, the store's layout and showroom-driven workflow make this task too arduous to be practical. After spending 2 hours in IKEA wandering among the showrooms, we finally picked out a dresser and retrieved it via aisle and part number among the warehouse section of the store, and were in the checkout line when we realized we had forgotten to get some extra cubby bins for another family member. But the thought of retracing our steps in the store, backtracking against the natural store's flow (seemingly one way) was too much to bear, and our energy had been exhausted. We left without the cubbies.
...
In the [Narrative Workflow Topics](2013/09/12/narrative-workflow-topics-helping-users-connect-the-dots-among-topics/) post, I took the approach of describing a workflow that users would proceed through, and then linked each of the tasks to the supporting documentation.

I wrote these posts a decade ago, and haven't done much with these more end-to-end and narrative posts much since. Part of the transition probably occurred with my shift into API documentation. In API docs, one treats the various endpoints as ingredients in a kitchen that can be used for myriad baking purposes. Hence the idea of a single user journey makes less sense. You can use the APIs in so many different combinations and workflows, we didn't want to restrict the creativity and flexibility of the code.
...
There is a kind of architect role becoming more common in large developer portals. Developer groups often realize that they have a large collection of independently documented APIs, products, and services, often intended to act as a unified and complete ecosystem for end-to-end needs, but without a clear path through all of them. In fact, when I was exploring different teams at Google, one of the roles was for this writer to trace solution paths through the many services in Google Cloud. It was a daunting task that would require you not only to be familiar with a wide number of technologies (not just programming languages but cloud infrastructure technologies), but also be familiar with the products and how they all interacted together into harmonious solutions. Who possessed such knowledge in an organization? Although appealing, it didn't seem like a great fit for someone just joining the organization. I didn't want to bite off more than I could chew.

However, I continue to be interested in the big picture. I expanded the sections in my API doc course with [Reasons why product overviews are often minimal or nonexistent](/learnapidoc/docapis_doc_overview.html#reasons_for_failure), also [giving a presentation](https://idratherbewriting.com/learnapidoc/docapis_doc_overview.html#presentation-on-product-overviews) on the same topic. I did the same for getting started tutorials, explaining [Reasons why getting started tutorials fail or don't exist](/learnapidoc/docapis_doc_getting_started_section.html#reasons_for_failure). I have been focusing on the big picture and how to get started probably because of my own discomfort in these areas &mdash; the products I work with are complex, and often vague. I want to get a better understanding of them and work with them in at least a minimal way myself.

However, getting this big picture understanding isn't something that comes easy. To understand the big picture, the forest instead of the trees, tech writers will need to sacrifice their focus on the small details of documentation. They will instead push this work onto the contributing engineering teams to work out and instead focus on how those details fit into the larger scope of the developer portal, where along the developer's journey the information fits, and whether the information has an interaction with the larger story and journey being told in the developer portal. The tech writer focuses his or her attention on the story that's being told, which is something that's too easy to forget when you're focused on the micro-level and concerned about whether parameters have proper descriptions or which file needs to be edited to make the product work properly.

The challenge is that rarely to tickets get filed about this larger story. The tickets (whether filed by product teams or from frustrated users) seem to always involve smaller technical details. Rarely does someone communicate that a product's purpose or description is unclear.
