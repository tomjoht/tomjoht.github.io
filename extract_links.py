...
image_link_regex = r'!\[([^\]]*)\]\(([^)]+)\)'
img_src_regex = r'<img\s+(?:[^>]*?\s+)?src="([^"]*)"[^>]*>' # For <img src="...">
liquid_tag_regex = r'\{\{[^}]+\}\}' # Basic regex for liquid tags

all_links = []

# Extract Markdown links
for match in re.finditer(markdown_link_regex, markdown_content):
    all_links.append(match.group(2))

# Extract HTML <a> tag links
for match in re.finditer(html_link_regex, markdown_content):
    all_links.append(match.group(1))

# Extract Markdown image links
for match in re.finditer(image_link_regex, markdown_content):
    all_links.append(match.group(2))
...
# Extract Liquid tags (as per instruction, just extract them for now)
for match in re.finditer(liquid_tag_regex, markdown_content):
    all_links.append(match.group(0))

# Remove duplicates and print as comma-separated string
unique_links = sorted(list(set(all_links)))
print(",".join(unique_links))
