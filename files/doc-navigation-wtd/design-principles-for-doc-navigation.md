...
When thinking about strategies for navigation within your documentation, it's helpful to start with universal design principles that have already been developed and studied, even if the design principles were discovered or studied in scenarios that don't scenarios outside of docs. (This is especially necessary when there isn't ample tech comm research on doc navigation patterns.)

In the book *Universal Principles of Design*, the authors explore design principles that they feel are universal and which apply across a wide variety of industries, products, and scenarios. It provides concise summaries about a range of design topics.

From 125 separate design principles, I selected out 5 that I think have particular application to document navigation. These principles include the following:
...
<a href="https://www.amazon.com/Universal-Principles-Design-William-Lidwell/dp/1592530079"><img src="../media/rasters/univprinciplesdesignbook.png"/></a>

Applying these principles in the way we design our docs can result in a better user experience.

## 1. Entry Point
The first principle of design I'd like to start with is the entry point.

<div class="bs-callout bs-callout-default"><b>Entry point:</b> A point of physical or attentional entry into a design. <a href="https://www.amazon.com/Universal-Principles-Design-William-Lidwell/dp/1592530079"><img src="../media/rasters/univprinciplesdesignbook.png"/></a></div>
...
Although the entry point for docs can be any page (as I explain later in [Modularity](#modularity)), I want to focus on the doc portal homepage, because it's one of the main entry points we design.

The doc portal entry point should list out documentation in a way that makes sense, such as by product and version, or by user role or goal. (This assumes that you have multiple sets of documentation.) The grouping should be logical as not to prevent or confuse people from getting started.

Think of the homepage as a train station. It should clearly direct users on where to go.
...
The homepage isn't a conceptual or explanatory page that describes your product, but rather directs users (both advanced, beginners, and other types) to the correct "terminals" for their documentation routes.
...
Additionally, quick reference guides or other getting started tasks would be appropriate on the homepage. Depending on the size of your help system, you will likely have a doc portal homepage followed by a product homepage. The getting started tutorials and quick reference guide tutorials would be more appropriate on the product homepage.

Overall, users come to the homepage to figure out where to go, to see what's available, and to get oriented. Try to make it as easy as possible for them to do get started. Users also start forming opinions and biases about the documentation from the entry point. If the entry point is disorganized and doesn't help them get started, the user's confidence is already crippled as they head into the rest of the documentation.

Another design principle to implement on your entry point is to formalize your desire lines. Desire lines are the natural paths that form based on frequently taken shortcuts users take.

## 2. Desire lines

Your homepage (particularly your product homepage) should also formalize the desire lines through your help.

<div class="bs-callout bs-callout-default"><b>Desire Line:</b> Traces of use or wear that indicate preferred methods of interaction with an object or environment.</div>
...
Here's an example from Twitter. With the most popular topics right at the top, they've formalized the desire lines to these topics.
...
<div class="bs-callout bs-callout-default"><b>Hierarchy:</b> Hierarchical organization is the simplest structure for visualizing and understanding complexity. </div>
...
Although the sidebar navigation is fairly common in docs, it also somewhat controversial. Some people think the sidebar nav is detritus from another era (e.g., dated tripane help models). They argue that it's not something people use, so why bother with it?

Actually, it turns out that if you remove the sidebar from your docs, users tend to dislike its absence. For example, in a comment on a post on my site, [Barry Grennon](https://www.linkedin.com/in/barrygrenon/) [explains](http://idratherbewriting.com/2017/05/01/write-the-docs-podcast-episode-5-where-do-we-belong/#comment-3288347593):
...
Invariably users interact with a help system in different ways. But in general, the sidebar menu grounds users with a sense of the whole, even if they don't always use that whole to navigate. If there isn't a "whole" to grasp, such as is the case with a knowledge base repository, the sidebar nav isn't necessary. But when there is a whole, users like having it visible at a glance.

## The necessity of seeing the whole

To better understand why we need this hierarchy sidebar nav, think of this sidebar as a list of parts. This list of parts lets users know what's contained in the whole.
...
With just about any complex system or object, to understand it, users break down the whole into a list of parts. It's the first step in wrangling complexity -- converting a complex entity into smaller parts to better understand what's going on. Think about some analogies here:
...
As you structure your sidebar, it's important to avoid deep hierarchies. Deep hierarchies are folders that contain folders that contain folders that contain folders, etc. Deep hierarchical nesting prevents users from grasping the whole at a glance, which is a primary function of the sidebar navigation.
...
<div class="bs-callout bs-callout-default"><b>Immersion:</b> A state of mental focus so intense that awareness of the "real" world is lost, generally resulting a feeling of joy and satisfaction.</div>

<img src="../media/vectors/wtd_doc_navigation_dontleavesystem.svg"/>

Although immersion mostly refers to removing distractions from the outside world so the user can become positively lost and engrossed within a system, forgetting the outside world, there are ties with documentation here. Users desire to be immersed in the application or system they're using rather than leaving that system to consult a separate, external system for help. Users feel like consulting the help (ironically) detracts them from the focus and progress toward their task.

The first way to cater to immersion, therefore, is to bring the help to the user. Don't require users to go to the help -- through context-sensitive features and other on-screen text, bring the help to users. This assumes there's a user interface for you to push the help. With developer docs, the help might best appear in the form of code comments, error messages, or interactive REST clients like Swagger.

But even when a user goes to your help system, the principle of immersion still applies. Users consuming content prefer to stay within that content rather than move back and forth interacting with a sidebar nav, which exists somewhat "outside" of the content.

<img src="../media/vectors/wtd_doc_navigation_usersclickinlinelinks.svg"/>

Immersed in a topic, users are more apt to click and navigate using the links within the topic itself. As such, you should provide navigational controls within the content instead rather than assuming all doc navigation will take place through the sidebar.

### Bottom-up navigation

The idea of navigating from within your content is called "bottom-up navigation" (rather than "top-down navigation," where users would drill down into the content through hierarchies). Bottom-up navigation is a principle that Mark Baker stresses on his site [Every Page Is Page One](http://everypageispageone.com/2015/02/10/bottom-up-information-architecture-q-and-a-part-1/).
...
These principles may seem like commonsense. In general, we link to topics as it makes sense to do so. But bottom-up navigation takes inline linking to a another level. Consider amplifying the number of links by two or three times so that your pages begin to resemble the inline links in wikipedia.

<img src="../media/rasters/wikipedia.png"/>

Amplifying the number of inline links create some challenges. First, it can be hard to author content when your text is littered with links. As you rename files, page titles, and headings, the links can easily break or list the wrong page titles. Finally, inline links create decision points in your content that force users to decide whether to continue reading or to jump to a new topic. This can result in users jumping around more in your content, which might [reduce readability and increase scattered-brainedness](http://idratherbewriting.com/2010/06/19/finally-convinced-about-removing-inline-links-to-increase-readability/).

But even with these challenges, don't neglect or minimize inline links. They're how many users simply navigate content. The web has taught us to navigate this way because our searches typically follow this pattern: we google something, land on a page, and then jump back to google's results, click to another page, click links within the page, and move to another page as we search for the right information. Even if we don't always operate in this mode, the many times we do operate in this mode condition us to operate similarly when using help material. We rarely navigate via the sidebar because we're jumping across different websites (clicking different google results) instead of moving within the same system.

This kind of hit-and-miss navigation through keyword searches and results pages is jumpy and nauseating in the long run and can't be sustained in the long run,. But because we have internalized the behavior as part of an Internet culture, we can't neglect this design pattern in our help material. Instead, we should build links into our content to help users navigate.
...
<div class="bs-callout bs-callout-default"><b>Modularity:</b> A method of managing system complexity that involves dividing large systems into multiple, smaller self-contained systems. </div>
...
Modularity also provides efficiencies for the authors, because it means they can work on an individual part without having to adjust the whole.
...
Most of us agree with this principle in theory, but it's hard to implement in practice, in part due to the information structures we may be required to follow.

### Don't assume building blocks == presentation

You may be writing in a documentation structure that separates content into tasks, concepts, and reference topics (e.g. DITA). If so, remember that these structures are a way of chunking content so you can re-use it. The re-use of content reduces your word count, lowers translation costs, avoids redundancy and duplication, and provides other efficiency benefits.

However, remember that these information chunks are building blocks only. Like legos, you can often arrange the blocks into more lengthy displays.

<img src="../media/vectors/usercentered_buildingblocks.svg"/>

You aren't required to have a two-sentence topic that stands on its own because it's a concept. You don't need a topic that is just 3 steps and nothing more because it's a "task." Users don't think in terms of topic types but rather goals and problems.

If you fragment your information into a [million little content pieces](http://idratherbewriting.com/2013/04/22/does-dita-encourage-authors-to-fragment-information-into-a-million-little-pieces/), you force users to jump around from topic to topic, like a pinball bouncing off of different walls and targets, looking for the information they need. That design leads to a disjointed, tiring experience. Instead, combine these chunks into more comprehensive, modular articles.

<img src="../media/rasters/cairns.png"/>

### Create longer, self-contained "articles"

Usually when you create standalone, modular topics, they end up being somewhat long. Don't be afraid of long topics. Some Wikipedia articles are so long they could be printed as ebooks. I like to think of documentation topics more like "articles," because the term <i>article</i> communicates the idea of standalone content that is complete unto itself.

Not all help information can stand <i>entirely</i> on its own. Frequently one concept or task builds on another topic, or requires knowledge of another topic. But there's usually a logical boundary line between topics. You can decide  how large to draw the boundaries. Just make sure users aren't bouncing from one topic to another trying to get answers for a single task or goal they're trying to achieve.

Although I shy away from actual word count numbers, a good-sized page can range anywhere from 800 to 3,000 words. It depends whether the topic is complete. Beyond 3,000 words, I usually start chunking the information.
...
<div class="bs-callout bs-callout-default"><b>Wayfinding: </b>The process of using spatial and environmental information to navigate to a testination.</div>

<img src="../media/vectors/wtd_doc_navigation_wayfinding.svg"/>

In documentation, wayfinding techniques might include providing contextual links, workflow maps, and breadcrumbs to help orient users as to where they are in a larger system.
...
As the user navigates the content, the on-page TOC lets a user see the content at a glance. It lets the user click to the section he or she is interested in, without requiring the user to manually scroll or skim the page for the information.
...
When you create the on-page TOC, make sure it's scannable. Avoid including too many levels, as it might detract from the user's ability to take in the content at a glance (the same principle as with the navigation sidebar).

Additionally, don't try to stuff the on-page TOC into the navigation sidebar (mixing heading links with links to other pages). Doing so unnecessarily balloons the sidebar with too many options, giving the impression that your documentation contains a *massive* set of pages to navigate and learn. (Users may often feel relief to realize that the sidebar links are actually just sections.) Users don't usually need all the information up front but can instead view more detail as they drill deeper into the site (this relates to another design principle &mdash; "progressive disclosure").
...
Even if you're not outlining a larger process or sequence, suggest a next move for the user. What else can/should they do?
...
At least a quart of the time when I try to cross this intersection, drivers get confused. Sometimes they drive half way through the intersection and then stop. I often see surprise on their faces as they realize, mid-way through, that they've just screwed up. This design violates what users have come to expect, and while it seemed like a neat idea, it doesn't work in practice.

When we follow universal design principles, we conform to the behaviors and patterns that work well with users. Our navigation features won't stand out but will rather invisibly support the main purpose of the help, allowing users to focus on the content and achieve their goals.
