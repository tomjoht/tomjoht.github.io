...
One of the most common tasks I do as a technical writer working with API documentation is to prepare biweekly release notes for several APIs. For these frequently releasing APIs, I create a Google Doc and ask teams to list details for the release notes. However, I also regenerate the reference documentation for the APIs, which leads to a diff that is highly valuable.

These are different sorts of APIs I work with &mdash; a cloud API, a Java API, and a gRPC API. It doesn't really matter what type of API it is because for each API, I rebuild the reference documentation (using [various scripts](/ai/prompt-engineering-doc-build-scripts.html)). When I rebuild the reference, I stage the content and see the file diff of everything that has changed with the previous release. This is one of the advantages of a docs-as-code environment &mdash; file diffs are possible for each change. (I'm more and more convinced that file diffs are one of the main advantages of docs-as-code systems.)

I look at this file diff and compare it with the release notes. Only about a quarter of the time do the manually gathered release notes match what I'm seeing in the file diff. For example, in a recent release, I noticed that several methods had been deprecated, but they weren't explicitly called out in the Google Doc. Someone had filed a bug about deprecating a few methods a few weeks ago but didn't indicate when it would go live. Another team just indicated that "various classes" were being deprecated. The file diff for the regenerated reference provided the definitive truth about the changes.

However, while the file diff is helpful, if the API has a lot of changes, reading the file diff can be confusing. You have to carefully look at what's been removed versus what's been added, assessing whether the changes are significant. Did someone just update a comment, or did they literally add or remove something?

By the way, I absolutely love being pulled into the reference material. I find that even among techie tech writers, only a fraction of them actually get their hands dirty with reference material. Understanding where to make changes in the source in order to influence the reference output involves a bit of know-how &mdash; even as simple as knowing which directories to search. Each time I get this diff, it provides a window into the correlation between the output and the source that I enjoy delving into.

I treat the reference as a source of truth for almost everything in the user guide material. If a method, parameter, or field is mentioned in the user guide material, I often link to it as a way of ensuring that it actually exists. A close relationship with reference material can be your best friend in tech comm scenarios. Without the reference, you're at the mercy of an engineer's word for information.

{% include ads.html %}

## Prompting scenario

Let's come back to the biweekly release. Different build systems will vary here, even among the different languages I mentioned, but most systems have a way of tagging a release build. This might be a snapshot of the code at a certain point in time. When you build the reference from this snapshot, the regenerated documentation provides a diff of what's changed between the current snapshot and the previous one. Even if you don't know what code changes have taken place in the release, it doesn't matter. When you generate the reference docs, the changes clearly present themselves in the diff because changes in the reference doc reflect changes in the code.

With the regenerated documentation, I stage my changes to see the file diff. At this point, I use a command (specific to my company's tools) to get a file diff printed out in my terminal about what's changed in the staged files. If you're using Git, you would use `git diff`, passing in the commit ID and files as desired.

The file diff printout in the terminal is hard to parse at a glance. It's too much work to sort through the changes line by line to identify significant ones. Here's where AI comes into play. Rather than try to parse the file diff, you can just have AI do it, providing a readable version that you can copy and paste into release notes if relevant.
...
You are a technical writer creating release notes for **[API NAME]**, a **[API DESCRIPTION]**. 

**TARGET AUDIENCE:**

The audience for the release notes consists of experienced [LANGUAGE] developers who integrate the **[API NAME]** into their applications. They rely on detailed documentation, advance notice of upcoming features, and clear information about potential breaking changes. 
...
Analyze the diff and identify the following changes using markdown formatting:

* **New features:** Describe any added classes, methods, or functionalities. Include their purpose and potential benefits, if you can extrapolate their purpose from the code. Explain how developers might use them.
* **Deprecations:** Identify any deprecated classes, methods, or fields. If the information is available, include an explanation of what they did, why they were deprecated, the impact on existing integrations, and recommended alternatives. 
...
Provide specific details, use markdown formatting, and link to relevant documentation or code examples if applicable.
...
* Keep the language concise and technical, targeting experienced Java developers.

**MARKDOWN SYNTAX:**

* For the title, use the header `# Release`.
* Use markdown headers (`##`, `###`, `####`) to structure the subheadings.
* Use bold (`**`) for class and method names.
...
The `processData()` method now uses a more efficient algorithm, resulting in a 20% performance improvement. This allows developers to process large datasets faster and improve the responsiveness of their applications.
```
...
Those are all the rules to keep in mind.
