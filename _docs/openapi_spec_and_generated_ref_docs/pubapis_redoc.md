...
The makers of Redoc spun off into their own company ([Redocly](https://redocly.com/)) and developed a more robust hosted solution offering (see [From ReDoc to Redoc.ly](https://redocly.com/blog/from-redoc-to-redocly)). As with both Stoplight and Swagger UI, Redoc offers both free and pro versions to adjust to your needs. And you can add topics beyond just reference docs.
...
2.  Change the value for `redoc spec-url` to a valid OpenAPI spec that is available at an online URL.
    ...
    If you were following the previous tutorial on creating an OpenAPI spec through Stoplight ([Getting started tutorial: Using Stoplight to create an OpenAPI specification document](pubapis_openapis_quickstart_stoplight.html)), you can use the OpenAPI URL that Stoplight provides. From your Stoplight display, go to the Overview topic and click **Export OpenAPI**. Choose either original or dereferenced. Copy the URL. Here's an [example URL](https://idratherbewriting.com/docs/openapi_spec_and_generated_ref_docs/openapi_openweathermap.yml).
